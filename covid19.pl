...
my $workingDirectory=$ENV{HOME}.$directorySeparator."Covid-19";
...
sub versionCheck{
	...
	if (!$check){
	 ...
	 if ($check){
		my $source="https://raw.githubusercontent.com/saiftynet/Covid19Epidemiology/master/covid19.pl";
		...
		if ($latest==$VERSION){
		  ...
		  my $input=prompt("Newer version exists\nDo you wish to",["Download latest version into working directory",
		                                     ...
		                                     "Skip this"]);
           getstore($source, $config{workingDirectory}.$directorySeparator."covid19.pl")  if $input =~/working directory/;
           getstore($source, "covid19.pl") if $input =~/here/;
		}
		else {
